...
module Unix = struct
  let of_fd x =
    ...
    if Sys.os_type <> "Unix" then
      Lwt.fail
        (Failure "Inheriting a listening socket is only supported on Unix")
...
module HyperV = struct
  let of_uri ~serviceid uri =
    ...
    let vmid =
      match Uri.host uri with None -> Hvsock.Loopback | Some x -> Hvsock.Id x
    in
    let serviceid =
...
let accept_forever ?backlog ~serviceid ~make_root t =
  let accept () =
    ...
    | `Tcp (host, port) ->
        ...
        Unix.accept_forever ?backlog socket (Unix.handle ~make_root t)
    | `HyperV_connect uri ->
        HyperV.connect_forever
          (HyperV.of_uri ~serviceid uri)
