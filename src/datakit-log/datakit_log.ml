...
type t = Quiet | Timestamp | Eventlog | ASL
...
let pp_ptime f () =
  ...
  Fmt.pf f "%04d-%02d-%02d %02d:%02d" (tm.tm_year + 1900) (tm.tm_mon + 1)
...
let reporter log_clock =
  let pp_time =
    match log_clock with `Posix -> pp_ptime | `Monotonic -> pp_mtime
  in
...
let log_clock =
  ...
  Arg.(
    value
    & opt (enum [ ("monotonic", `Monotonic); ("posix", `Posix) ]) `Posix doc)
