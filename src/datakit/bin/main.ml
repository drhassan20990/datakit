...
module Git_fs_store = struct
  ...
  let listener =
    lazy (Irmin.Private.Watch.set_listen_dir_hook Irmin_watcher.hook)
...
let start ~listen_9p prometheus git =
  ...
  let serve_9p =
    ...
    >|= fun make_root ->
    List.map
      (fun addr -> Datakit_conduit.accept_forever ~make_root ~serviceid addr)
      listen_9p
  in
  ...
  Lwt_main.run
    ...
    | Some remote -> (
        ...
        | Some local ->
            ...
            Git_fs_store.Store.Branch.watch_all repo (fun branch _ ->
                Autopush.push pusher ~branch;
                Lwt.return_unit)
            >>= fun w ->
            start () >>= fun () ->
...
let auto_push =
  let docs = git_options in
  let doc =
    Arg.info ~doc:"Auto-push the local repository to a remote source." ~docs
      ~docv:"URL" [ "auto-push" ]
  in
  Arg.(value & opt (some string) None doc)
...
let term =
  ...
  in
  ( Term.(
      pure start $ setup_log $ listen_9p $ Prometheus_unix.opts $ git
      $ auto_push),
