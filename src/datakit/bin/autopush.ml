...
let exec ~name cmd =
  ...
  | Unix.WEXITED 0 -> ()
  | Unix.WEXITED i -> Log.err (fun l -> l "%s exited with code %d" name i)
  | Unix.WSIGNALED i -> Log.err (fun l -> l "%s killed by signal %d)" name i)
  | Unix.WSTOPPED i -> Log.err (fun l -> l "%s stopped by signal %d" name i)
  ...
  let rec loop () =
    ...
    | dirty ->
        ...
        Lwt.catch
          (fun () ->
            let cmd =
              [ "git"; "-C"; t.local; "push"; "--force"; "--"; t.remote ]
              @ dirty
            in
            let name =
              Fmt.strf "auto-push: %a" (Fmt.Dump.list String.dump) cmd
            in
...
let create ~local ~remote =
  ...
  let t =
    { local; remote; dirty = String.Set.empty; cond = Lwt_condition.create () }
  in
