...
module Vfs (Store : Store.S) = struct
  ...
  module CommitListFile : sig
    ...
    type t = { repo : Store.Repo.t; file : Vfs.File.t; get : unit -> string }
  ...
  let irmin_ro_file ~get_root path =
    let read () =
      ...
      | Some (`Node _) -> err_is_dir
      | Some (`Contents (f, _perm)) -> ok (Some (Blob.to_ro_cstruct f))
    in
  ...
  let irmin_rw_file ~remove_conflict ~view path =
    ...
    | Some (dir, leaf) ->
        ...
        in
        let blob_or_empty () =
          blob () >>*= function None -> ok Blob.empty | Some b -> ok b
        in
        let remove () =
        ...
        let open_ () =
          let read ~offset ~count =
            blob () >>*= function
            | None -> err_no_entry
            | Some b -> Lwt.return (Blob.read ~offset ~count b)
  ...
  let name_of_irmin_path ~root path =
    match Path.rdecons path with None -> root | Some (_, leaf) -> leaf
  ...
  let remove_shadowed_by items map =
    List.fold_left (fun acc (name, _) -> String.Map.remove name acc) map items
  ...
  let rw ~conflicts view =
    ...
    and irmin_rw_dir path =
      ...
      let ls () =
        ...
        (Store.Tree.find_tree root path >>= function
         | None | Some (`Contents _) ->
             Lwt.return [] (* in parent's extra_dirs? *)
      ...
      let mkfile name perm =
        ( match perm with
        | (`Normal | `Exec) as perm ->
            Dir.update view path name (empty_file, perm)
      ...
      let lookup name =
        let real_result =
          ...
          | Some (`Node _ as dir) -> (
              let step = Store.Key.v [ name ] in
              Store.Tree.kind dir step >>= function
              | Some ((`Contents | `Node) as ty) ->
                  ok (get ~dir:path (name, ty))
              | None -> err_no_entry )
        in
  ...
  module Transaction : sig
    val make :
      Store.t ->
      info:(string -> Irmin.Info.t) ->
      remover:unit Lwt.t Lazy.t ->
      Vfs.Dir.t Lwt.t
    ...
    type t = {
      repo : Store.Repo.t;
      store : Store.t;
      (* The branch we're going to commit on *)
      view : Dir.t;
    ...
    let commit_of_view ~info t =
      ...
      let msg =
        match t.get_msg () with "" -> "(no commit message)" | x -> x
      in
    ...
    let make store ~info ~remover =
      ...
      let origin =
        Vfs.File.ro_of_string (Fmt.to_to_string Store.Key.pp path)
      in
      let diff = Diff.vfs_dir repo (fun () -> Lwt.return (Dir.root t.view)) in
      let common =
        empty_inode_map |> add stage
      ...
      in
      let normal_mode () =
        common |> add (Vfs.Inode.file "merge" (Vfs.File.command merge_mode))
      in
  ...
  let reflog_stream store =
    ...
    let remove_watch =
      let cb = function
        | `Added x | `Updated (_, x) ->
            Vfs.File.Stream.publish session (Some x)
  ...
  let rec watch_dir store ~path =
    ...
    let cache = ref empty_inode_map in
    (* Could use a weak map here *)
    let lookup name =
      match String.Map.find name !cache with
      | Some x -> x
      | None ->
          let new_path = Path.rcons path name in
  ...
  let transactions ~info store =
    ...
    let lookup name =
      match String.Map.find name !items with
      | Some x -> ok x
      | None -> err_no_entry
    in
    let remover name =
    ...
    let mkdir name =
      Lwt_mutex.with_lock lock (fun () ->
          if String.Map.mem name !items then Vfs.Dir.err_already_exists
            ...
            if Lazy.is_val remover then err_no_entry
            else
  ...
  let branch_dir ~info repo =
    ...
    let ls () =
      ...
      let names =
        ...
        String.Map.bindings !cache |> List.map fst |> StringSet.of_list
        |> StringSet.union names |> StringSet.elements
      in
    ...
    let rename inode new_name =
      ...
      | false -> (
          ...
          | Some entry ->
              snd entry new_name;
              cache :=
                !cache |> String.Map.remove old_name
    ...
    let read () =
      (Store.Head.find store >>= function
       ...
       | Some head -> (
           ...
           try Store.History.pred hist head with Invalid_argument _ -> [] ))
      >|= fun parents ->
