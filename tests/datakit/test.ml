...
module RW_err1 = struct
  ...
  let pp fmt = function `Not_a_directory -> Fmt.string fmt "Not_a_directory"
...
let test_writes () =
  let ( >>*= ) x f =
    x >>= function Ok y -> f y | Error _ -> Alcotest.fail "9p protocol error"
  in
  let v = ref "" in
  let read () = Lwt.return (Ok (Some (Cstruct.of_string !v))) in
  let write x =
    v := Cstruct.to_string x;
    Lwt.return (Ok ())
  in
    ...
    ( Vfs.File.open_ file >>*= fun h ->
      let check src off expect =
        ...
        Alcotest.(check string)
          (Printf.sprintf "After %S at %d" src off)
          expect !v;
        Lwt.return_unit
      in
...
let test_rw () =
  ...
  Lwt_main.run
    ( Store.Repo.v config >>= fun repo ->
      ...
      >|= Alcotest.(check (result unit err)) "Write /a" (Ok ())
      >>= fun () ->
      ...
      >|= Alcotest.(check (result unit err1))
            "rm /foo/bar"
            (Error `Not_a_directory)
      >>= fun () ->
...
let test_blobs_random () =
  ...
  Alcotest.check (vfs_result reject) "Read negative"
    (Vfs.Error.negative_offset (-1L))
    (Datakit.Blob.read b ~offset:(-1L) ~count:1);
  Alcotest.check (vfs_result reject) "Read after EOF"
    (Vfs.Error.offset_too_large ~offset:6L 5L)
    (Datakit.Blob.read b ~offset:6L ~count:1);
  ()

let test_streams () =
  let ( >>*= ) x f =
    x >>= function Ok y -> f y | Error _ -> Alcotest.fail "VFS error"
  in
  Lwt_main.run
...
let test_conduit () =
  ...
  List.iter check
    [ (`Tcp ("localhost", 1234), "tcp://localhost:1234", Same);
      (`Tcp ("localhost", 123), "tcp://localhost", Expect "tcp://localhost:123");
...
let () =
  Alcotest.run "datakit"
    [ ("server", test_set); ("conduit", [ ("basic", `Quick, test_conduit) ]) ]
