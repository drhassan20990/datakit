...
class main t =
  object (self)
    ...
    method private render rd =
      ...
      let csrf_token = CI_web_utils.Session_data.csrf_token session_data in
      Wm.continue
        (CI_web_templates.main_page ~csrf_token ~ci:t.ci
...
class metrics t =
  object (self)
    ...
    method content_types_provided rd =
      Wm.continue [ ("text/plain; version=0.0.4", self#to_plain) ] rd

    method content_types_accepted rd = Wm.continue [] rd

    method! is_authorized rd =
      match Cohttp.Header.get_authorization rd.Rd.req_headers with
      | Some (`Other token) when check_metrics_token t.server token ->
          Wm.continue `Authorized rd
      | _ -> Wm.respond ~body:(`String "Bad token") 403 rd

    method private to_plain rd =
      let data =
        Prometheus.(CollectorRegistry.collect CollectorRegistry.default)
      in
      let body =
...
class pr_list t =
  object
    ...
    method private render rd =
      Wm.continue (CI_web_templates.prs_page ~ci:t.ci) rd
  end
...
class branch_list t =
  object
    ...
    method private render rd =
      Wm.continue (CI_web_templates.branches_page ~ci:t.ci) rd
  end
...
class tag_list t =
  object
    ...
    method private render rd =
      Wm.continue (CI_web_templates.tags_page ~ci:t.ci) rd
  end
...
class ref_page t =
  object (self)
    ...
    method private render rd =
      ...
      let csrf_token = CI_web_utils.Session_data.csrf_token session_data in
      Wm.continue
        (CI_web_templates.target_page ?test ~csrf_token ~target state)
        rd
  end
  ...
  object
    ...
    method private render rd =
      ...
      let archived_targets =
        live_targets
        |> List.fold_left
             (fun acc target -> CI_target.Map.remove target acc)
             archived_targets
      ...
      | [ t ], [] ->
          Wm.respond 307
            (Rd.redirect (Uri.to_string (CI_target.path ?test t)) rd)
      | _ ->
          Wm.continue
            (CI_web_templates.commit_page ~commit ~archived_targets ?test
               live_targets)
            rd
  end
...
type graphics_state = { bold : bool; fg : string option; bg : string option }
...
let process_escapes ~gfx_state ~buf data =
  ...
  let rec aux s =
    ...
    | `Invalid i -> aux i
    | `Escape (`Reset, i) ->
        gfx_state := default_gfx_state;
        aux i
    | `Escape (`Ctrl (`SelectGraphicRendition c), i) ->
        gfx_state := List.fold_left apply_ctrl !gfx_state c;
        aux i
  in
  ...
  object (self)
    ...
    method private to_html rd =
      ...
      | Some live_log -> (
          ...
          let template =
            Fmt.to_to_string (Tyxml.Html.pp ()) (html ~user web_config)
          in
...
class saved_log_page t =
  object (self)
    ...
    method private to_html rd =
      ...
      | Error e ->
          let html =
            CI_web_templates.plain_error (Fmt.to_to_string DK.pp_error e)
          in
          let body =
            Fmt.to_to_string (Tyxml.Html.pp ()) (html ~user web_config)
          in
                    ...
                    (fun () ->
                      let rec aux i =
                        if i = Cstruct.len log_data then (
                          ...
                          let data =
                            Cstruct.sub log_data i len |> Cstruct.to_string
                          in
                          let data = process_escapes ~gfx_state ~buf data in
                          (if data = "" then Lwt.return () else out#push data)
                          >>= fun () ->
...
module Settings = struct
  class index t =
    object
      ...
      method private render rd = Wm.continue CI_web_templates.Settings.index rd
    end
end
...
let routes ~logs ~ci ~server ~dashboards =
  ...
  [ (* Auth *)
    ...
    ( "auth/github-callback",
      fun () -> new CI_web_utils.github_callback t.server );
    ...
    ( "settings/github-auth",
      fun () -> new CI_web_utils.github_auth_settings t.server );
    (* Errors *)
    ...
    (* Static resources *)
    ( ":dir/:name",
      fun () -> new CI_web_utils.static_crunch ~mime_type CI_static.read )
  ]
