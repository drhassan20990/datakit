...
module Term : sig
  ...
  val state :
    'a t -> ('a, [ `Pending of string | `Failure of string ]) result t
  (** [state x] immediately and successfully returns the current state
      of [x]. *)

  val of_state :
    ('a, [< `Pending of string | `Failure of string ]) result -> 'a t
  ...
  val branch_head :
    Datakit_github.Repo.t -> string -> Datakit_github.Commit.t t
...
module Cache : sig
  ...
  module Make (B : BUILDER) : sig
    ...
    val lookup :
      t -> (unit -> DK.t Lwt.t) -> B.context -> B.Key.t -> B.value status Lwt.t
