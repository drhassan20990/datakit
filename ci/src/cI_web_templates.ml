...
let config ?(name = "datakit-ci") ?state_repo ?metrics_token
...
let log_commit_url t commit = state_repo_url t "commit/%s" commit

let log_branch_history_url t branch = state_repo_url t "commits/%s/log" branch

let log_branch_results_url t branch = state_repo_url t "tree/%s" branch
...
let gh_target_url = function
  ...
  | `Ref (repo, id) -> (
      ...
      match id with
      | "tags" :: id ->
          Fmt.strf "https://github.com/%s/%s/releases/tag/%s" user repo
            (String.concat ~sep:"/" id)
      | _ ->
          Fmt.strf "https://github.com/%s/%s/tree/%a" user repo Ref.pp_name id
      )
...
let commit_history_url t target ~metadata_commit ~src_commit =
  ...
  let user = User.name user in
  state_repo_url t "commits/%s/%s/%s/commit/%s" metadata_commit user repo
    src_commit
...
let short_commit x = String.with_range ~len:8 x

let encode = Uri.pct_encode ~scheme:"http"

let saved_log_frame_link ~branch ~commit =
  Printf.sprintf "/log/saved/%s/%s" (encode branch) (encode commit)

let logs_link = function
  | `Live live_log ->
      Printf.sprintf "/log/live/%s" (encode (CI_live_log.branch live_log))
  | `Saved { CI_output.branch; commit; _ } ->
      saved_log_frame_link ~branch ~commit
...
let pr_map f map =
  PR.Index.fold (fun key value acc -> f key value @ acc) map []
...
let status state =
  ...
  in
  span
    ~a:[ a_class [ "label"; colour ] ]
    [ span ~a:[ a_class [ "glyphicon"; icon ] ] []; txt status ]
...
let summarise jobs =
  ...
  let combine status outputs =
    ...
    let pp_group f (descr, g) =
      Fmt.pf f "%s (%a)" descr
        Fmt.(list ~sep:(const string ", ") Fmt.string)
        (List.rev g)
    in
    let descr =
      Fmt.strf "%a" Fmt.(list ~sep:(const string "; ") pp_group) results
    in
    CI_result.v status descr
  in
  let pending, outputs =
    List.partition (fun (_, s) -> CI_output.status s = `Pending) outputs
  in
  if pending <> [] then combine `Pending pending
  else
    let failed, outputs =
      List.partition (fun (_, s) -> CI_output.status s = `Failure) outputs
    in
    if failed <> [] then combine `Failure failed else combine `Success outputs
...
let dashboard_widget (_repo, id) ref =
  ...
  in
  let title = match id with ("heads" | "tags") :: tl -> tl | x -> x in
  div
    ~a:[ a_class [ "col-md-4"; "text-center"; "dashboard"; cls ] ]
...
let ref_job (_repo, id) ref =
  ...
  | jobs ->
      ...
      [ tr
          [ td
              [ a ~a:[ a_href ref_url ]
                  [ txt (Fmt.to_to_string Ref.pp_name id) ]
              ];
            td [ status_list jobs ];
...
let pr_job (_repo, id) open_pr =
  ...
  | jobs ->
      let summary = summarise jobs in
      let pr_url =
        Uri.to_string (CI_target.path_v (CI_engine.target open_pr))
      in
      [ tr
...
let url x = a ~a:[ a_href x ] [ txt x ]

module Nav = struct
  type t = Home | PRs | Branches | Tags | Settings
...
let page page_title active children t ~user =
  ...
  let nav_header =
    nav
      ~a:[ a_class [ "navbar"; "navbar-inverse"; "navbar-fixed-top" ] ]
      [ div
          ~a:[ a_class [ "container" ] ]
          [ div
              ~a:[ a_class [ "navbar-header" ] ]
              [ button
                  ~a:
                    [ a_user_data "toggle" "collapse";
                      a_user_data "target" "#navbar";
                      a_class [ "navbar-toggle" ];
            ...
            div
              ~a:[ a_id "navbar"; a_class [ "collapse"; "navbar-collapse" ] ]
              [ ul ~a:[ a_class [ "nav"; "navbar-nav" ] ] navbar;
                ul
                  ~a:[ a_class [ "nav"; "navbar-nav"; "navbar-right" ] ]
  ...
  let content =
    [ div
      ...
      script
        ...
        (txt "");
      script
        ~a:[ a_mime_type "text/javascript"; a_src "/js/bootstrap.min.js" ]
        (txt "")
    ]
  in
...
let opt_warning ci =
  ...
  | Lwt.Return _ -> []
  | Lwt.Sleep ->
      [ div ~a:[ a_class [ "warning" ] ] [ txt "Connecting to DataKit..." ] ]
  | Lwt.Fail ex ->
      let msg =
        Fmt.strf "DataKit connection is down: %s" (Printexc.to_string ex)
      in
...
let dashboard_row acc x =
  let l, r = x in
  let col1 =
    match l with None -> div ~a:[ a_class [ "col-md-4" ] ] [] | Some l -> l
  in
  let col2 =
    match r with None -> div ~a:[ a_class [ "col-md-4" ] ] [] | Some r -> r
  in
  div
    ~a:[ a_class [ "row" ] ]
...
let html_of_user ~csrf_token ((job, label), log) =
  ...
  let log_link =
    ...
    | Some log ->
        [ txt "[ ";
          a ~a:[ a_href (logs_link (`Live log)) ] [ txt "log" ];
          txt " ] "
        ]
    | None -> []
  in
  let branch = Uri.pct_encode ~scheme:"http" branch in
  let query = [ ("CSRFToken", [ csrf_token ]) ] in
  let action =
    Printf.sprintf "/cancel/%s?%s" branch (Uri.encoded_of_query query)
  in
  let target, job_name = job in
  let link = Uri.to_string (CI_target.path target) in
  let reason =
    Fmt.strf "%a:%s%a" CI_target.pp target job_name pp_opt_label label
  in
  [ br ();
    form
      ~a:[ a_class [ "cancel" ]; a_action action; a_method `Post ]
      [ button
          ~a:
...
let resource_pools ~csrf_token =
  let items =
    String.Map.bindings (CI_monitored_pool.pools ())
    |> List.map (fun (name, pool) ->
  ...
  table
    ~a:[ a_class [ "table"; "table-bordered"; "table-hover" ] ]
    (tr [ th [ txt "Name" ]; th [ txt "Utilisation" ] ] :: items)
...
let login_page ?github ~csrf_token state ~is_configured t ~user =
  ...
  let github_login =
    match github with
    | None -> p [ txt "(configure webauth to allow GitHub logins)" ]
    | Some github ->
        p
      ...
      [ div
          ...
          [ txt
              "The CI has not yet been configured with an administrator user. \
               In the CI's logs you should find a URL containing a token - \
               open this URL to create an 'admin' user."
          ]
      ]
  in
  page "Login" Nav.Home ~user
    ( [ h2 [ txt "Login" ];
        CI_form.Html.form ~csrf_token state ~form_class:[ "login-form" ]
          ~action
          [ field "Username" `Text "user";
            field "Password" `Password "password";
            div
              [ button
                  ~a:[ a_class [ "btn"; "btn-primary" ]; a_button_type `Submit ]
                  [ txt "Log in" ]
              ]
          ];
        github_login
...
let auth_setup ~csrf_token state =
  ...
  page "Auth Setup" Nav.Home
    [ CI_form.Html.form state ~csrf_token ~form_class:[ "auth-setup-form" ]
        ~action
        [ div
          ...
          button
            ~a:[ a_class [ "btn"; "btn-default" ]; a_button_type `Submit ]
            [ txt "Submit" ]
        ]
    ]
...
let user_page ~csrf_token =
  ...
  page "Profile" Nav.Home
    [ form
        ~a:[ a_class [ "logout-form" ]; a_action action; a_method `Post ]
        [ button
            ~a:[ a_class [ "btn"; "btn-default" ]; a_button_type `Submit ]
            [ txt "Log out" ]
        ]
...
let main_page ~csrf_token ~ci ~dashboards =
  ...
  @ [ h2 [ txt "Resource pools" ]; resource_pools ~csrf_token ]
...
let prs_page ~ci =
  ...
  let title_of_project (id, _) = Fmt.to_to_string Repo.pp id in
  let title =
    "CI: " ^ (List.map title_of_project prs |> String.concat ~sep:" / ")
  in
...
let branches_page ~ci =
  ...
  let title_of_project (id, _) = Fmt.to_to_string Repo.pp id in
  let title =
    "CI: " ^ (List.map title_of_project refs |> String.concat ~sep:" / ")
  in
...
let history_button url =
  a
    ~a:[ a_class [ "btn"; "btn-default"; "btn-sm" ]; a_href url ]
    [ span ~a:[ a_class [ "glyphicon"; "glyphicon-time" ] ] []; txt "History" ]

let log_button_group history log_url =
  div
    ~a:[ a_class [ "btn-group"; "pull-right" ] ]
    [ history;
      a
        ~a:[ a_class [ "btn"; "btn-default"; "btn-sm" ]; a_href log_url ]
        [ span ~a:[ a_class [ "glyphicon"; "glyphicon-book" ] ] [];
          txt "Artefacts"
        ]
    ]
    ...
    | Live live_log ->
        ...
        [ form
            [ status_flag `Pending;
              button
                ~a:
                  [ a_class [ "btn"; "btn-default"; "btn-xs"; "rebuild" ];
                    a_button_type `Submit;
                    a_disabled ()
                  ]
                [ span
                    ~a:
                      [ a_class
                          [ "glyphicon"; "glyphicon-refresh"; "pull-left" ]
                      ]
                    [];
                  txt "Rebuild"
                ];
...
let job_row ?selected ~csrf_token ~page_url (job_name, state) =
  ...
  in
  let attrs =
    if selected = Some job_name then [ a_class [ "selected-job" ] ] else []
  in
  tr ~a:attrs
    [ th [ txt job_name ];
      td [ status (CI_output.status output) ];
      td (logs ~csrf_token ~page_url output)
    ]
...
let commit_page ?test ~commit ~archived_targets targets t =
  ...
  let target_link target =
    ...
    li [ a ~a:[ a_href uri ] [ txt (Fmt.to_to_string CI_target.pp target) ] ]
  in
  let archive_target_link (target, commit) =
    let commit = CI_utils.DK.Commit.id commit in
    let uri =
      Uri.to_string
        (Uri.add_query_param' (CI_target.path ?test target) ("history", commit))
    in
    li [ a ~a:[ a_href uri ] [ txt (Fmt.to_to_string CI_target.pp target) ] ]
  in
  page title Nav.Home
...
let history_nav t target state =
  ...
  let links =
    [ txt " [ ";
      a ~a:[ a_href "?" ] [ txt "latest" ];
      txt " | ";
      a ~a:[ a_href history_link ] [ txt "history" ];
      txt " ]"
    ]
  in
  ...
  | xs ->
      p
        ( txt "Previous states: "
          :: intersperse (txt ", ") (List.map state_link xs)
        @ links )

let target_page ?test ~csrf_token ?(title = "(no title)")
    ~(target : CI_target.t) state t =
  let jobs =
    CI_history.State.jobs state
    |> String.Map.bindings
    |> List.map (fun (name, s) -> (name, Some s))
  in
  let title = target_title ~title target in
  let repo = CI_target.repo target in
  let src_commit =
    CI_history.State.source_commit state |> CI_utils.default "MISSING-COMMIT"
  in
  let metadata_commit =
    CI_history.State.metadata_commit state |> CI_utils.default "MISSING-COMMIT"
  in
  let page_url = target_page_url target in
  let state_summary =
    [ table
        ~a:[ a_class [ "table"; "table-bordered"; "results" ] ]
        (List.map (job_row ?selected:test ~csrf_token ~page_url) jobs)
    ]
  in
  let nav =
    match target with
  ...
  page title nav
    ...
    :: p
         [ a ~a:[ a_href (gh_target_url target) ] [ txt title ];
           txt " has head commit ";
           a
             ~a:[ a_href (commit_url ~repo src_commit) ]
             [ txt (short_commit src_commit) ];
           txt " [ ";
           ...
           a
             ~a:
               [ a_href
                   (commit_history_url t target ~metadata_commit ~src_commit)
               ]
             [ txt "status history for this head" ];
           txt " ]"
         ]
    :: state_summary )
    t
...
let live_log_frame ~branch ~have_history t ~user:_ =
  ...
  iframe_page ~page_title
    (body
       ...
       [ div
           ~a:[ a_class [ "row" ] ]
           [ div ~a:[ a_class [ "col-md-9" ] ] [ p [ txt "Still running..." ] ];
             div
               ~a:[ a_class [ "col-md-3" ] ]
    ...
    (body
       ...
       [ div
           ...
           [ div
               ~a:[ a_class [ "col-md-9" ] ]
               [ p
                   [ txt "Loaded from commit ";
                     a ~a:[ a_href commit_url ] [ txt commit ]
                   ]
               ];
             div
               ~a:[ a_class [ "col-md-3" ] ]
...
let error_page id =
  page "Error" Nav.Home
    ( if id = Error.no_state_repo then
      [ p
          [ txt
              "No web mirror of the state repository has been configured, so \
               can't link to it."
          ];
        p
          [ txt
              "Configure DataKit to push to a GitHub repository and then pass \
               the repository's URL using ";
            code [ txt "Web.config ~state_repo" ];
            txt "."
          ]
      ]
    else if id = Error.permission_denied then [ p [ txt "Permission denied" ] ]
    else if id = Error.logout_needed then
      [ p
          [ txt
              "Access policy has changed - please log out and log back in so \
               we can check your credentials against the new policy."
          ]
      ]
    else [ p [ txt (Printf.sprintf "Unknown error code %S" id) ] ] )
...
module Settings = struct
  let index =
    page "Settings" Nav.Settings
      [ ul
          [ li
              [ a
                  ~a:[ a_href "/settings/github-auth" ]
                  [ txt "Configure GitHub authentication" ]
              ]
          ]
      ]
  ...
  let github_auth ~csrf_token state =
    ...
    page "GitHub authentication" Nav.Settings
      [ div
          ~a:[ a_class [ "github-auth" ] ]
          [ h2 [ txt "GitHub authentication" ];
            p
              [ txt
                  "You can configure the CI to allow users to authenticate \
                   using their GitHub accounts. To do this:"
              ];
            ol
              [ li
                  [ p
                      [ txt "Go to ";
                        ...
                        txt " and register your CI. "
                      ];
                    p
                      [ txt
                          "Give the callback URL as \
                           <https://HOST:PORT/auth/github-callback> (you can \
                ...
                li
                  [ p [ txt "Enter the Client ID and Client Secret below." ];
                    p
                      [ txt
                          "You can also specify a callback here, if you want \
                           to override the one you registered with GitHub \
                           above. This might be useful if you have several CI \
                           instances using a single Client ID."
                      ]
                  ]
              ];
            CI_form.Html.form ~csrf_token state ~form_class:[ "github-auth" ]
              ~action
