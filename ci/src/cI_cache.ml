...
module Metrics = struct
  ...
  let builds_started_total =
    ...
    Counter.v_label ~help ~label_name:"name" ~namespace ~subsystem
...
module Make (B : CI_s.BUILDER) = struct
  module Cache : sig
    ...
    type entry = { lock : Lwt_mutex.t; mutable value : B.value status option }

    type t = { mutable cache : entry String.Map.t }
    ...
    let with_lock t k fn = Lwt_mutex.with_lock (entry t k).lock fn
  end
  ...
  let mark_for_rebuild t conn branch_name =
    ...
    | Some { result = Error (`Pending _); _ } ->
        Lwt.return () (* If already building, ignore rebuild request *)
    | _ ->
  ...
  let load_from_db t conn key =
    ...
    | Some commit -> (
        ...
        | true ->
            Lwt.return None (* Results exist, but are flagged for rebuild *)
        | false -> (
            ...
            | Ok (`Success data) ->
                Some { result = Ok data; output = logs ~failed:false }
            | Ok (`Failure _ as f) ->
                Some { result = Error f; output = logs ~failed:true }
            | Error (`Failure msg) ->
                Log.err (fun f ->
                    f
  ...
  let do_build t conn ctx k =
    ...
    let log =
      CI_live_log.create ~switch ~pending:title t.logs ~branch:branch_name
        ~title
    in
    CI_live_log.heading log "%s" title;
    CI_live_log.log log "Starting...";
    let pending_thread = monitor_pending t branch_name log in
    (* Sets initial state too *)
    let finish result =
    ...
    Lwt.async (fun () ->
        Lwt.catch
          (fun () ->
            ...
            conn () >>= fun dk ->
            DK.branch dk branch_name >>*= fun branch ->
            Prometheus.Gauge.track_inprogress
              (Metrics.builds_in_progress (B.name t.builder))
            @@ fun () ->
            Prometheus.Summary.time
              (Metrics.build_time (B.name t.builder))
              Unix.gettimeofday
            @@ fun () ->
            DK.Branch.with_transaction branch (fun trans ->
                ...
                | Ok x ->
                    ...
                    DK.Transaction.create_or_replace_file trans Path.log
                      (Cstruct.of_string (CI_live_log.contents log))
                    >>*= fun () ->
                ...
                | Error (`Failure msg) ->
                    ...
                    DK.Transaction.create_or_replace_file trans Path.log
                      (Cstruct.of_string (CI_live_log.contents log))
                    >>*= fun () ->
                    DK.Transaction.create_file trans Path.failure
