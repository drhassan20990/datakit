...
module Make (C : CI_s.CONTEXT) = struct
  ...
  let pp_names ppf names =
    if List.length names > 10 then Fmt.pf ppf "%d tasks" (List.length names)
    else Fmt.(list ~sep:(const string ", ") string) ppf names
  ...
  let wait_for_all l =
    ...
    | ps, fs ->
        pending "%a failed (still waiting for %a)" pp_names fs pp_names ps
end
