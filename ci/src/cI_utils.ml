...
let with_switch fn =
  ...
  Lwt.finalize (fun () -> fn switch) (fun () -> Lwt_switch.turn_off switch)
...
let ls path =
  let rec read_files acc fd =
    Lwt.try_bind
      ...
      (function End_of_file -> Lwt.return acc | ex -> Lwt.fail ex)
  in
  Lwt_unix.opendir path >>= fun fd ->
  Lwt.finalize (fun () -> read_files [] fd) (fun () -> Lwt_unix.closedir fd)
