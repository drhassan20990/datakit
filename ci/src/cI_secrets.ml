...
let ensure_crt ~private_key path =
  if Sys.file_exists path then Lwt.return ()
    ...
    let data =
      X509.Encoding.Pem.Certificate.to_pem_cstruct1 crt |> Cstruct.to_string
    in
    Lwt_io.with_file ~mode:Lwt_io.output path (fun ch -> Lwt_io.write ch data)
...
let const value =
  let conv =
    object
      method read = assert false

      method write = failwith "const"
    end
  in
  let lock = Lwt_mutex.create () in
  { value; conv; lock }
