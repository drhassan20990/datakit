...
(* A problem that is expected to resolve itself with time *)
...
let pp ok f = function Ok x -> ok f x | Error e -> pp_error f e

let descr = function Ok x -> x | Error (`Failure x | `Pending x) -> x
...
let of_json = function
  ...
  | `Assoc [ ("status", `String "pending"); ("descr", `String d) ] ->
      Error (`Pending d)
  | `Assoc [ ("status", `String "failure"); ("descr", `String d) ] ->
      Error (`Failure d)
  | json ->
      CI_utils.failf "Invalid results JSON: %a"
