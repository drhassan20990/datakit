  ...
  let pp =
    Fmt.braces (fun f { parents; jobs; source_commit; metadata_commit } ->
        Fmt.pf f "parents = %a;@ jobs = %a;@ source = %a;@ meta = %a"
          ...
          (String.Map.dump (CI_output.pp Fmt.string))
          jobs
...
type t = { cache : (string, target) Hashtbl.t (* Branch name -> target *) }
...
let load commit =
  ...
  (DK.Tree.read_dir tree (Path.of_steps_exn [ "job" ]) >>= function
   | Ok items ->
       items
...
let lookup t dk target =
  ...
  | t ->
      Lwt_mutex.with_lock t.lock (fun () -> Lwt.return t)
      (* Ensures we've finished loading *)
  | exception Not_found ->
      let target =
...
let record t dk ~source_commit input jobs =
  ...
  if
    String.Map.is_empty patch && state.State.source_commit = Some source_commit
  then (
    ...
    DK.Branch.with_transaction branch (fun tr ->
        ...
        DK.Transaction.create_or_replace_file tr source_commit_path
          source_commit
        >>*= fun () ->
        DK.Transaction.create_or_replace_file tr metadata_commit_path
          metadata_commit
        >>*= fun () ->
        String.Map.bindings patch
        ...
        ( match !messages with
        | [] ->
            DK.Transaction.commit tr ~message:"Update"
            (* e.g. no statuses or new source commit *)
        | [ message ] -> DK.Transaction.commit tr ~message
        | m :: _ as ms ->
            let message =
              Fmt.strf "%d updates (%s, ...)@.@.%a" (List.length ms) m
    ...
    | Some state_commit ->
        ...
        let sub_name =
          Fmt.to_to_string CI_target.Branch_escape.pp_sub target
        in
        let message =
          Fmt.strf "Update %s:%s -> %s" sub_name source_commit state_commit
        in
