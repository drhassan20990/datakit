...
module State = struct
  ...
  let empty_field = { data = None; error = None }
  ...
  let bindings t = Multipart.StringMap.bindings !t
...
module Html = struct
  ...
  let field state descr ty name =
    ...
    let err =
      ...
      | None -> []
      | Some err ->
          [ div ~a:[ a_class [ "alert"; "alert-danger" ] ] [ txt err ] ]
    in
  ...
  let form state ~csrf_token ~form_class ~action children =
    ...
    let warnings =
      ...
      |> List.map (fun (name, field) ->
             let err =
               field.State.error |> CI_utils.default "Unexpected field"
             in
             div
...
module Validator = struct
  type 'a t =
    data ->
    string Multipart.StringMap.t ->
    ('a, string Multipart.StringMap.t) result

  type 'a reader = string -> 'a or_error

  let maybe v _ acc =
    if Multipart.StringMap.is_empty acc then Ok v else Error acc

  let fail field ~msg _ acc = Error (Multipart.StringMap.add field msg acc)
  ...
  let non_empty = function "" -> Error "Cannot be empty" | s -> Ok s

  let uri x =
    try Ok (Uri.of_string x) with ex -> Error (Printexc.to_string ex)

  let confirm required actual =
    if required <> actual then Error "Values don't match!" else Ok ()

  let optional v = function
    | "" -> Ok None
    | x -> ( match v x with Ok x -> Ok (Some x) | Error _ as e -> e )

  let ( >>!= ) x f parts acc =
    match x parts acc with Ok x -> f x parts acc | Error _ as e -> e
