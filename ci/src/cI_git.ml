...
let with_gitdir dir env =
  let not_gitdir s = not (String.is_prefix ~affix:"GIT_DIR=" s) in
  let env =
    Printf.sprintf "GIT_DIR=%s" dir
    :: (env |> Array.to_list |> List.filter not_gitdir)
  in
  Array.of_list env
...
module Commit = struct
  ...
  let is_after ~old t =
    ...
    with Not_found ->
      let result =
        let cmd =
          ("", [| "git"; "merge-base"; "--is-ancestor"; old; t.hash |])
        in
        CI_process.run_with_exit_status ~cwd:t.repo.dir ~output:ignore cmd
        >|= function
        | Unix.WEXITED 0 -> true
        | Unix.WEXITED _ -> false
        | x ->
            CI_process.check_status cmd x;
            true
      in
...
module Builder = struct
  module Key = struct
    ...
    let pp f = function `PR pr -> PR.pp f pr | `Ref r -> Ref.pp f r

    let head = function `PR pr -> PR.commit pr | `Ref r -> Ref.commit r
  ...
  let name t = "git:" ^ t.dir

  let title _t key = Fmt.strf "Git fetch %a" Key.pp key
  ...
  let generate t ~switch ~log _trans NoContext target =
    ...
    let env =
      Unix.environment () |> with_gitdir (Filename.concat t.dir ".git")
    in
      ...
      (fun ex ->
        ...
        let cmd =
          [| "git";
             "fetch";
             "origin";
             Printf.sprintf "%s:tags/%s" (Key.branch target) tmp_tag
          |]
        in
        CI_process.run ~switch ~env ~output ("", cmd) >>= fun () ->
        Lwt_mutex.with_lock git_lock @@ fun () ->
        (* (hopefully fetch can handle parallel uses) *)
        CI_process.run ~env ~output
          ("", [| "git"; "branch"; "-f"; tmp_branch; hash |])
        >>= fun () ->
...
type ready_t = { repo : repo; fetches : PR_Cache.t }

type t = ready_t Lwt.t

let is_directory d =
  match Sys.is_directory d with r -> r | exception Sys_error _ -> false
...
let clone_if_missing ?remote ~dir =
  if is_directory dir then Lwt.return ()
    ...
    | None ->
        CI_utils.failf
          "Directory %S does not exist (and no ~remote provided, so can't \
...
let fetch_head t target =
  ...
  CI_term.target target >>= PR_Cache.find t.fetches Builder.NoContext
...
let with_checkout ~log ~job_id { Commit.repo = { dir; dir_lock; _ }; hash } fn
  ...
  CI_live_log.enter_with_pending_reason log
    ...
    (CI_monitored_pool.use ~log dir_lock job_id)
  @@ fun () ->
  let output = CI_live_log.write log in
  Lwt_mutex.with_lock git_lock (fun () ->
...
module Shell_builder = struct
  ...
  let name t = "shell:" ^ t.label

  let title t commit = Fmt.strf "Run %s on commit %a" t.label Commit.pp commit
