...
(* Branch -> current live log *)

type stream = { data : string; next : stream option Lwt.t Lazy.t }

let create_manager () = ref String.Map.empty
...
let create ?switch ~pending ~branch ~title manager =
  ...
  | Some existing ->
      CI_utils.failf
        "Can't create new live log on branch %S; one is already active!\n%s"
        branch
        (Buffer.contents existing.buffer)
  | None ->
...
let stream t =
  let rec loop offset =
    ...
    let send () =
      ...
      Lwt.return
      @@ Some { data = Bytes.unsafe_to_string data; next = lazy (loop length) }
    in
...
let printf t fmt = Fmt.kstrf (write t) fmt
...
let log t fmt =
  ...
  |> Fmt.kstrf @@ fun msg ->
     ...
     write t
       Unix.(
         Fmt.strf "[%04d-%02d-%02d %02d:%02d.%02d] %s\n" (tm.tm_year + 1900)
...
let pending t =
  let next = if t.finished then `Stop else `Continue t.pending_updated in
