...
module Workflows = struct
  ...
  let ls_clone =
    Git.command ~timeout:60.0 ~label:"ls" ~clone:true [ [| "ls" |] ]
...
let test_simple conn =
  Test_utils.with_ci conn Workflows.test_circleci_artifact
  @@ fun ~logs ~switch dk with_handler ->
  DK.branch dk "github-metadata" >>*= fun hooks ->
  let wait_for ~commit path =
    Test_utils.wait_for_file ~switch hooks
      (Printf.sprintf "user/project/commit/%s/status/%s" commit path)
  in
  (* A new PR appears *)
  ...
  Test_utils.assert_file hooks
    "user/project/commit/123/status/ci/datakit/test/description"
    "Waiting for ci/circleci status to appear"
  >>= fun () ->
  with_handler ~logs "http://circle/build/50"
    ~pending:"CircleCI ready; enqueued pitfall job" (fun ~switch:_ _log ->
      (* A pending CircleCI build appears *)
      Test_utils.update_pr hooks ~message:"CircleCI pending" ~id:3907
        ~head:"123"
        ~states:
      ...
      wait_for ~commit:"123" "ci/datakit/test/description"
        ~old:"Waiting for ci/circleci status to appear"
        "Waiting for ci/circleci to complete"
      >>= fun () ->
      (* CircleCI finishes the build *)
      Test_utils.update_pr hooks ~message:"CircleCI ready" ~id:3907 ~head:"123"
        ~states:[ ("ci/circleci/state", "success") ]
      >>= fun () ->
  ...
  | Some head ->
      ...
      DK.Tree.read_file tree
        (Datakit_client.Path.of_string_exn "job/test/output")
      >>*= fun data ->
...
let test_branch conn =
  Test_utils.with_ci conn Workflows.pass
  @@ fun ~logs ~switch dk with_handler ->
  DK.branch dk "github-metadata" >>*= fun hooks ->
  let wait_for ~commit path =
    Test_utils.wait_for_file ~switch hooks
...
let test_circle_rebuild conn =
  ...
  with_handler ~logs "http://cirlce/build/50"
    ~pending:"CircleCI ready; enqueued pitfall job" (fun ~switch:_ _log ->
      (* A new PR appears with a successful CircleCI build *)
      Test_utils.update_pr hooks ~message:"Init" ~id:3907 ~head:"123"
        ~states:
  ...
  with_handler ~logs "http://cirlce/build/51"
    ~pending:"CircleCI ready; enqueued pitfall job" (fun ~switch:_ _log ->
      Test_utils.update_pr hooks ~message:"Rebuilt CircleCI" ~id:3907
        ~head:"123"
        ~states:
          [ ("ci/circleci/state", "success");
...
let test_logs conn =
  Test_utils.with_ci conn Workflows.simple_parallel
  @@ fun ~logs ~switch dk with_handler ->
  DK.branch dk "github-metadata" >>*= fun hooks ->
  let wait_for ~commit path =
    Test_utils.wait_for_file ~switch hooks
...
let expect_pending = function
  | Error (`Pending x) -> x
  | Error (`Failure msg) ->
      Alcotest.fail ("Expected Pending, got Failure " ^ msg)
...
let test_git_dir conn ~clone =
  let logs = Private.create_logs () in
  let cmd = if clone then Workflows.ls_clone ~logs else Workflows.ls ~logs in
  ...
  (* Start the CI *)
  Test_utils.with_ci conn (Workflows.pull_and_run local_repo ~cmd)
  @@ fun ~logs ~switch dk with_handler ->
  DK.branch dk "github-metadata" >>*= fun hooks ->
  let wait_for ~commit path =
    Test_utils.wait_for_file ~switch hooks
  ...
  | Some head ->
      DK.Commit.tree head >>*= fun tree ->
      DK.Tree.read_file tree (Datakit_client.Path.of_string_exn "log")
      >>*= fun log ->
...
let test_git_tag conn =
  ...
  let local_repo = Git.v ~remote:remote_dir ~logs local_clone in
  (* Start the CI *)
  Test_utils.with_ci conn Workflows.(fetch_only ~local_repo)
  @@ fun ~logs ~switch dk with_handler ->
  DK.branch dk "github-metadata" >>*= fun hooks ->
  let wait_for ~commit path =
    Test_utils.wait_for_file ~switch hooks
  ...
  with_handler ~logs "a" (fun ~switch:_ _log ->
      Test_utils.update_ref hooks ~message:"Tag" ~id:"v0.1" ~head:hash
        ~states:[]
      >>= fun () ->
      wait_for ~commit:hash "ci/datakit/test/state" "pending" >>= fun () ->
      Lwt.return (Ok "Success"))
...
let test_cross_project conn =
  ...
  @@ fun ~logs ~switch dk _with_handler ->
  ignore logs;
  DK.branch dk "github-metadata" >>*= fun hooks ->
  let wait_for ~commit path =
    Test_utils.wait_for_file ~switch hooks
  ...
  Test_utils.wait_for_file ~switch hooks
    "user/project/commit/123/status/ci/datakit/test/description"
    "Compile 123 with 654" ~old:"Compile 123 with 987"
  >>= fun () ->
  (* PR changes *)
  Test_utils.update_pr hooks ~message:"Init" ~id:3907 ~head:"456" ~states:[]
  >>= fun () ->
  Test_utils.wait_for_file ~switch hooks
    "user/project/commit/456/status/ci/datakit/test/description"
    "Compile 456 with 654"
...
let with_test_auth fn =
  ...
  Utils.with_tmpdir (fun tmp ->
      ...
      output_string ch
        "((admin((prf \
         SHA1)(salt\"\\172~\\212>|'\\154\\202\\224\\128?\\158\\160\\245\\243j\")(hashed_password\"_\\231gB\\136\\221\\159!\\164%\\024\\\"0H\\\"\\230\\172\\142\\166\\138\")(count \
         5000)(dk_len 20))))";
      close_out ch;
...
let test_roles conn =
  ...
  let server ~public =
    let can_read =
      if public then CI_ACL.everyone else CI_ACL.username "admin"
    in
    let can_build = CI_ACL.username "admin" in
    let web_config =
      CI_web_templates.config ~name:"test-ci" ~can_read ~can_build ()
    in
    let server =
      CI_web_utils.server ~web_config ~auth ~session_backend:`Memory
        ~public_address:web_ui
    in
    let routes =
      CI_web.routes ~server ~logs ~ci ~dashboards:(CI_target.map_of_list [])
    in
    fun ~expect path ->
...
let read_to stream expect =
  let rec aux ~sofar =
    ...
    | None ->
        Alcotest.fail
          (Fmt.strf "End-of-stream waiting for %S@.Got: %s" expect sofar)
    | Some next ->
...
let test_live_logs conn =
  ...
  let log =
    CI_live_log.create ~pending:"Building" ~branch ~title:"Test" logs
  in
  ...
  read_to log_page "TEST-<span class='fg-bright-red bold'>OUTPUT</span>-1"
  >>= fun () ->
  ...
  | Some (code, header, _body, _path) ->
      ...
      let path =
        Cohttp.Header.get header "location"
        |> Test_utils.or_fail "Missing location"
      in
...
let test_history conn =
  ...
  let target_t =
    (module CI_target : Alcotest.TESTABLE with type t = CI_target.t)
  in
  let commit_t =
    (module DK_Commit : Alcotest.TESTABLE with type t = DK_Commit.t)
  in
  ...
  (* Initially empty *)
  Alcotest.check (Alcotest.option state_t) "Empty head" None
    (CI_history.head master_h);
  ...
  |> CI_history.State.jobs
  |> Alcotest.check jobs_t "Initial commit" s1;
  CI_history.record master_h dk ~source_commit metadata_commit s1 >>= fun () ->
  CI_history.head master_h
  |> Test_utils.or_fail "No head state!"
  ...
  (* Check s2 *)
  let head2 =
    CI_history.head master_h |> Test_utils.or_fail "No head state!"
  in
  let head1 =
  ...
  Alcotest.(check (option string))
    "Source commit" (Some "123")
    (CI_history.State.source_commit loaded1);
  let s1_expected =
  ...
  CI_history.builds_of_commit dk head >|= CI_target.Map.bindings
  >>= fun builds ->
  DK.branch dk (CI_target.status_branch master) >>*= DK.Branch.head
  >>*= fun expected_head ->
  let expected_head =
    expected_head |> Test_utils.or_fail "Missing status branch"
  in
  Alcotest.(check (list (pair target_t commit_t)))
    "Non-empty index"
    [ (master, expected_head) ]
    builds;
  Lwt.return ()
...
let test_process () =
  Lwt_main.run
    (let out = Buffer.create 40 in
     ...
     let cmd =
       ("/bin/sh", [| "/bin/sh"; "-c"; "echo output; echo error >&2" |])
     in
     Datakit_ci.Process.run cmd ~output:(Buffer.add_string out)
       ~stderr:(Buffer.add_string err)
     >>= fun () ->
     Alcotest.(check string) "stdout" "output\n" (Buffer.contents out);
     Alcotest.(check string) "stderr" "error\n" (Buffer.contents err);
     Datakit_ci.Process.run cmd ~output:(Buffer.add_string out) >>= fun () ->
     Alcotest.(check string)
       "stdout" "output\noutput\nerror\n" (Buffer.contents out);
     Lwt.return ())
...
let () = Alcotest.run "datakit-ci" [ ("ci", test_set) ]
