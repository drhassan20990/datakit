...
module Dockerfile = struct
  ...
  let self_ci =
    v ~timeout:(30. *. minute) "ci/self-ci/Dockerfile"
      ~label:"Dockerfile.self-ci"
...
let projects repo = [ Config.project ~id:"moby/datakit" (Tests.datakit repo) ]

let can_build =
  let open ACL in
  any [ username "admin"; github_org "moby" ]

let make_config ?state_repo ~listen_addr ~remote () =
  let repo = Git.v ~logs ~remote "/data/repos/datakit" in
  let web_config =
    Web.config ~name:"datakit-ci" ?state_repo
      ~github_scopes_needed:[ `Read_org ] ~can_read:ACL.everyone ~can_build
      ~listen_addr ()
  in
...
let config_for = function
  | `Production ->
      make_config ~remote:"https://github.com/moby/datakit.git"
  ...
  | `Localhost ->
      ...
      make_config ~remote:"/mnt/datakit" ~listen_addr:(`HTTP 8080)
        (* Don't bother with TLS when testing locally. *) ()
...
let profile =
  let doc =
    Arg.info [ "profile" ] ~docv:"PROFILE"
      ~doc:"Which configuration profile to use."
  in
  Arg.(value @@ opt (enum profiles) `Production doc)

let () = Datakit_ci.run Cmdliner.Term.(pure config_for $ profile)
