...
let tests _target = [ ("my-test", my_test) ]
...
let projects =
  [ Config.project
      ~id:
        "me/my-project"
        (* The project is at https://github.com/me/my-project *)
      ~dashboards:
        [ "master" ] (* Key branches to display in the dashboard overview *)
      tests
...
let web_config =
  Web.config ~name:"example-ci"
    ~can_read:ACL.(everyone)
    ~can_build:ACL.(username "admin")
    ?state_repo ~listen_addr:(`HTTPS 8443) ()
