...
let src =
  Logs.Src.create "bridge-local-git" ~doc:"Local Git bridge for Datakit"

module Log = (val Logs.src_log src : Logs.LOG)

let src9p =
  Logs.Src.create "bridge-local-git.9p"
    ~doc:"Local Git bridge for Datakit (9p)"
...
let repo_id =
  let parse s =
    ...
    | None ->
        `Error
          (Fmt.strf "Bad repository name %S (format should be user/project)" s)
    | Some x -> `Ok x
  in
...
let repos =
  let doc =
    Arg.info []
      ~doc:
        "A Git repository to monitor and the name to use for it. e.g. \
         'my/my-project:/tmp/my-project'"
      ~docv:"NAME:PATH"
  in
...
let main =
  ...
  let man =
    [ `S "DESCRIPTION";
      `P
        "$(tname) is a local replacement for datakit-github. It allows you to \
         test DataKitCI against a local Git repository without having to \
