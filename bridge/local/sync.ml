...
let src =
  Logs.Src.create "bridge-local-git.sync"
    ~doc:"Local Git bridge sync for Datakit"

module Log = (val Logs.src_log src : Logs.LOG)

module Make (S : Irmin.S with type branch = string) (DK : Datakit_client.S) =
struct
  ...
  let on_change t repo_id irmin_repo branch =
    ...
    let next =
      ...
      | Some head ->
          Ref.Index.add id
            (Commit.v repo_id (Fmt.to_to_string S.Commit.pp head))
            old
    in
  ...
  (* We never make merge transactions. *)
  ...
  let update_ref tr ~changelog ~new_state existing_ref =
    ...
    | None ->
        ...
        Buffer.add_string changelog
          (Fmt.strf "Removing deleted branch %a@." Ref.pp existing_ref);
    ...
    | Some new_head ->
        ...
        if Commit.equal new_head (Ref.commit existing_ref) then Lwt.return ()
          ...
          Buffer.add_string changelog
            (Fmt.strf "Updating existing branch to %a@." Ref.pp r);
  ...
  let sync t new_state =
    ...
    DK.Branch.with_transaction t.metadata_branch (fun tr ->
        ...
        Lwt_list.iter_s
          (update_ref tr ~changelog ~new_state)
          (Ref.Set.elements old_refs)
        >>= fun () ->
        let new_state = !new_state in
        Lwt_list.iter_s (add_ref tr ~changelog) (Ref.Index.bindings new_state)
        >>= fun () ->
  ...
  let run dk repos =
    ...
    let t =
      { repos = monitored; metadata_branch; known = Ref.Index.empty; cond }
    in
