...
module Make (API : API) = struct
  ...
  type token = { t : API.token; c : Capabilities.t }
  ...
  let status_of_commits token commits =
    let api_status token c =
      ...
      if not (Capabilities.check token.c `Read `Commit) then
        ok Status.Set.empty
      else
        ...
        | Ok status ->
            let status =
              List.filter
                (fun s -> Capabilities.filter_elt token.c `Read (`Status s))
                status
            in
  ...
  let read_prs token ids =
    ...
    >|= fun new_prs ->
    List.fold_left
      (fun new_prs -> function Ok (Some pr) -> PR.Set.add pr new_prs
  ...
  let read_refs token ids =
    ...
    >|= fun new_refs ->
    List.fold_left
      (fun new_refs -> function Ok (Some r) -> Ref.Set.add r new_refs
  ...
  let apply token diff =
    ...
    Lwt_list.iter_p (api_set_pr token) (PR.Set.elements prs) >>= fun () ->
    ...
    Lwt_list.iter_p (api_set_ref token) (Ref.Set.elements refs) >>= fun () ->
  ...
  let import_webhook_events token ~events t =
    ...
    | events ->
        ...
        status_of_commits token new_commits >|= fun new_status ->
        let events =
          (List.map Event.of_status @@ Status.Set.elements new_status) @ events
        in
