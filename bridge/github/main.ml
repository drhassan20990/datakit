...
let jar_path =
  try List.find Sys.file_exists jar_paths with Not_found -> List.hd jar_paths
...
let () =
  Lwt.async_exception_hook :=
    fun exn -> Logs.err (fun m -> m "Unhandled exception: %a" Fmt.exn exn)

type endpoint = [ `File of string | `Tcp of string * int | `Git of string ]
...
type datakit_config = { endpoint : endpoint; branch : string }
...
let connect_9p ~token ?webhook ?resync_interval ~cap ~branch ~repositories
  ...
  | Ok conn -> (
      ...
      | Ok br ->
          ...
          Sync.sync ~token ?webhook ?resync_interval ~cap br t >|= ignore )
...
let connect_git ~token ?webhook ?resync_interval ~cap ~branch ~repositories
  ...
  | Ok br ->
      ...
      Sync_git.sync ~token ?webhook ?resync_interval ~cap br t >|= ignore
...
let connect ~token ?webhook ?resync_interval ~repositories ~cap d =
  ...
  | `Git root ->
      connect_git ~token ?webhook ?resync_interval ~cap ~branch ~repositories
        root
...
let start () datakit repositories cap webhook resync_interval prometheus =
  ...
  set_signal_if_supported Sys.sigterm
    (Sys.Signal_handle
       (fun _ ->
...
let setup_log =
  let env =
    Arg.env_var ~docs:env_docs
      ~doc:"Be more or less verbose. See $(b,--verbose)." "DATAKIT_VERBOSE"
  in
  Term.(
    const Datakit_log.setup $ Fmt_cli.style_renderer ()
...
let branch =
  ...
  let doc =
    Arg.info ~docs
      ~doc:
        "DataKit/GitHub branch. Reflect the GitHub state. Writes to this \
         branch will be translated into    GitHub API calls."
      [ "b"; "branch" ]
  in
...
let resync_interval =
  ...
  let doc =
    Arg.info ~docs
      ~doc:
        "Interval to wait before doing full resyncrhonisation of the \
         repositories."
      [ "resync-interval" ]
  in
...
let resync =
  Term.(
    pure (fun i -> function false -> Some i | true -> None)
    ...
    Arg.info ~docs
      ~doc:
        "A comma-separated list of API capabilities, for instance \
         `*:r,status:rw` to allow the read of all resources but only the \
         write of build status."
      [ "c"; "capabilities" ]
  in
...
let term =
  ...
  let man =
    [ `S "DESCRIPTION";
      `P
        "$(tname) exposes a subset of the GitHub API as a 9p filesystem. Also \
         connect to a DataKit instance and ensure a bidirectional mapping \
         between the GitHub API and a Git branch."
    ]
  in
  ( Term.(
      pure start $ setup_log $ datakit $ repositories $ capabilities $ webhook
      $ resync $ Prometheus_unix.opts),
