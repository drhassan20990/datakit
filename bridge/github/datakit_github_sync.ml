...
module Make (API : API) (DK : Datakit_client.S) = struct
  ...
  *)
  ...
  let pp_state ppf t =
    Fmt.pf ppf "@[<h2>bridge:@ %a@]@;@[<2>datakit:@ %a@]" Snapshot.pp t.bridge
      Conv.pp t.datakit
  ...
  let create ~debug ?old br =
    let bridge =
      match old with None -> Snapshot.empty | Some o -> o.bridge
    in
    let old = match old with None -> None | Some o -> Some o.datakit in
    ...
    let init =
      ...
      | None ->
          DK.Branch.with_transaction br (fun tr ->
              ...
              DK.Transaction.create_or_replace_file tr file data >>= function
  ...
  let update_datakit ?(retries = 5) t tr =
    let rec aux n =
      ...
      if commited then Lwt.return t
        ...
        if n = retries then aux (n + 1) else Lwt.return t )
    in
    aux 1
  ...
  let continue = function Some s -> Lwt_switch.is_on s | None -> true
  ...
  let process_webhook = function
    ...
    | Some w ->
        ...
        let rec wait s =
          ...
          wait s
        in
        let run s = Lwt.pick [ API.Webhook.run w; wait s ] in
    ...
    | `Repeat ->
        ...
        let rec react () =
          if not (continue switch) then Lwt.return_unit
            ...
            ( if !updates = `None then Lwt_condition.wait cond
        ...
        in
        Lwt.choose [ react (); timer (); watch br; webhook () ] >|= fun () ->
        !t
