...
type token = { m : unit Github.Monad.t; token : Github.Token.t }
...
module Ref = struct
  ...
  let to_list s =
    match String.cuts ~empty:false ~sep:"/" s with "refs" :: l | l -> l
...
module Event = struct
  ...
  let of_gh ~token e =
    let repo =
      match String.cut ~sep:"/" e.event_repo.repo_name with
...
let user_exists token ~user =
  ...
  try
    token.m
    >>+= (fun () -> Github.User.info ~user ())
    |> run
    >|= R.map (fun _ -> true)
  with Github.Message _ -> Lwt.return (Ok false)

let repo_exists token { Repo.user; repo } =
  let user = User.name user in
  try
    token.m
    >>+= (fun () -> Github.Repo.info ~user ~repo ())
    |> run
    >|= R.map (fun _ -> true)
  with Github.Message _ -> Lwt.return (Ok false)
...
let set_status token status =
  ...
  token.m
  >>+= (fun () -> Github.Status.create ~user ~repo ~sha ~status:new_status ())
  |> run >|= R.map ignore
...
let set_pr token pr =
  ...
  token.m
  >>+= (fun () -> Github.Pull.update ~user ~repo ~num ~update_pull:new_pr ())
  |> run >|= R.map ignore
...
module Webhook = struct
  ...
  type t = { token : token; hook : Hook.t }
  ...
  let events t =
    Lwt_list.map_p
      (fun (r, e) -> event_hook_constr ~token:t.token (to_repo r) e)
      (Hook.events t.hook)

  let repos t =
    Hook.repos t.hook |> Github_hooks.Repo.Set.elements |> List.map to_repo
    |> Repo.Set.of_list
